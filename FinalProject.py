...
@app.route('/login/')
def showLogin():
    """This function returns the login page associated by a state code."""
    # Genertate a state code.
    state = ''.join(random.choice(string.ascii_uppercase + string.digits)
                    for x in xrange(32))
    # Store the state code in the session vriable.
    session['state'] = state
...
def gconnect():
    """This function is responsible for connecting Google Aouth and get user
     data."""

    # Check if the user state match the server state.
    ...
    # Get the flow code.
    code = request.data
    # Exchange it with the Credentials
    try:
        ...
        response = make_response(json.dumps(
            'Faild to upgrade the authorization code.'), 401)
        response.headers['Content-Type'] = 'application/json'
        return response
    ...
    if result['sub'] != gplus_id:
        response = make_response(json.dumps(
            "Token's user id dosn't match given user id."), 401)
        response.headers['Content-Type'] = 'application/json'
        return response
    if result['aud'] != CLIENT_ID:
        response = make_response(json.dumps(
            "Token's user id dosn't match app;s id."), 401)
    ...
    if stored_credentials is not None and gplus_id == stored_gplus_id:
        response = make_response(json.dumps(
            "The user's logged in aleardy."), 200)
        response.headers['Content-Type'] = 'application/json'
        return response
    ...
    # Prepare a welcome message for the user.
    output = ''
    output += '<h1>Welcome, '
...
def fbconnect():
    ...
    # Request user name, id, email && picture.
...
def gdisconnect():
    """Resposible for logging a user out who was using google Aouth."""
...
def fbdisconnect():
    """Resposible for logging a user out who was using Facebook Aouth."""
    ...
    if request.method == 'POST':
        ...
        if len(request.files) != 0:
            ...
            new_Genre = Genre(name=request.form['name'],
                              description=request.form['description'],
                              user_id=session['user_id'],
                              image="missing%d" % random.randint(1, 7))
            session_db.add(new_Genre)
            session_db.commit()
        return redirect(url_for('showGenres'))
    else:
...
def showMovie(genre_id, movie_id):
    """this page will display movie %s in the genre %s""" % (
        movie_id, genre_id)
...
def editMovie(genre_id, movie_id):
    ...
    if request.method == 'POST':
        ...
        return render_template('editMovie.html',
                               genre_id=genre_id, movie=edited_movie)
